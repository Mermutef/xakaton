# recommend.py – финальная версия с динамическим целевым месяцем
import pandas as pd
import numpy as np
import joblib
import warnings
...
# Целевой месяц – из переменной окружения или по умолчанию март 2026
target_str = os.environ.get("TARGET_MONTH", "2026-03-01")
TARGET_MONTH = pd.Timestamp(target_str)

print(f"Целевой месяц: {TARGET_MONTH.strftime('%Y-%m')}")
...
def compute_features_current():
    print(f"Вычисление признаков на {TARGET_MONTH.strftime('%Y-%m')}...")
...
result_df = result_df.merge(
    current_features[['ЛС', 'cluster', 'months_debt']],
    on='ЛС', how='left'
)

result_df.to_csv('data/recommendations.csv', index=False)
print("Рекомендации сохранены в data/recommendations.csv")

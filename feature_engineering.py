...
def build_static_features(general_df):
    ...
    # Адрес (ГУИД) оставим пока как категорию, позже можно заменить на кластер или частотную кодировку
    df['ГУИД'] = df['Адрес (ГУИД)']
    # Удалим ненужные
...
def build_master_table():
    ...
    # Заполняем пропуски для тех, кого нет в некоторых таблицах
    master = master.fillna(0)
    # Для категориальных признаков, которые не участвовали, можно оставить 0
    return master
